				...
				<div class="col-lg-12">
					<div class="content-area">
						<form id="payment-form" method="POST" class="checkoutform" action="{{route('Offline.submit')}}">
							...
							<button type="submit" style="display: none;" id="pay-btn" class="mybtn1 checkout-btn">{{ $langg->lang162 }}</button>
...
<script type="text/javascript">
	...
	$('#add-to-cart-form select').on('change', function() {
		$('#add-product-to-cart').prop('disabled', false);
	});
	
	$('#add-product-to-cart').on('click', function(){
		$(this).prop('disabled', true);
		form_data = $('#add-to-cart-form').serialize();
		$.ajax({
	...
	$(document).on('click','.increase-product-quantity-ajax',function(){
    ...
    $(document).on('click','.decrease-product-quantity-ajax',function(){
